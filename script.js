...
// À placer après la déclaration des variables, dans la fonction init() ou dans un DOMContentLoaded
function initSidebarAndViews() {
  const sidebar = document.getElementById('sidebar');
  const overlay = document.getElementById('sidebarOverlay');
  const menuToggle = document.getElementById('menuToggle');
  const closeSidebar = document.getElementById('sidebarCloseMobile');
  const navItems = document.querySelectorAll('.sidebar-item[data-view]');
  const views = {
    dashboard: document.getElementById('viewDashboard'),
    document: document.getElementById('viewDocument'),
    history: document.getElementById('viewHistory'),
    clients: document.getElementById('viewClients'),
    templates: document.getElementById('viewTemplates'),
  ...
  function openView(viewName) {
    ...
    // Rafraîchir le contenu des vues dynamiques
    ...
    if(viewName === 'clients' && typeof renderClientsPanel === 'function') renderClientsPanel();
    if(viewName === 'templates' && typeof openTemplateModal === 'function') {
      // Appeler la fonction qui remplit la grille (mais on évite la modale)
      const grid = document.getElementById('templateGrid');
      if(grid && grid.children.length === 0) {
        // Re-créer les templates (vous pouvez appeler votre fonction existante)
        if(typeof window.loadTemplates === 'function') window.loadTemplates();
        else {
          // Code simple pour remplir les templates (à adapter)
          const TEMPLATES = [ /* vos templates */ ];
          TEMPLATES.forEach(tpl => { /* créer les cartes */ });
        }
      }
    }
  }
  ...
  // Par défaut, afficher la vue document
  openView('document');
}
...
// Appeler ces deux fonctions après avoir chargé le reste de l'application
// Par exemple, à la fin de votre fonction init() existante :
async function init() {
  // ... votre code actuel (supabase, auth, etc.)
  initSidebarAndViews();
  initTheme();
  // ... le reste
}
